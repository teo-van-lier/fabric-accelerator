...
X = X.groupby('Date').agg(
    transaction_count=('CustomerID', 'count'),  # counts all transactions
    total_amount=('Amount', 'sum')             # sums the amount
).sort_index()

# Fill in missing days with 0
X = X.asfreq('D', fill_value=0)
X = X.reset_index()
target_col = re.sub('[^A-Za-z0-9_]+', '_', "total_amount")
...
mlflow.set_experiment("predict-actuals-AutoMLExperiment")
...
settings = {
    ...
    "force_cancel": True, 
    "seed": 41 , # Random seed 
    "mlflow_exp_name": "predict-actuals-AutoMLExperiment",  # MLflow experiment name
...
with mlflow.start_run(nested=True, run_name="predict-actuals"):
    automl.fit(
        X_train=X_train, 
...
registered_model = mlflow.register_model(model_uri=model_path, name="predict-actuals")
...
saved_name = "actuals_predictions".replace(".", "_")
predictions.write.mode("overwrite").format("delta").option("overwriteSchema", "true").save(f"Tables/actuals_predicted")
